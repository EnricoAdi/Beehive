    ...
    <div class="flex justify-center h-screen">
        <div class="mt-8">
            <div class="block px-6 pb-6 rounded-lg shadow-lg bg-secondary max-w-md mt-4 w-96">
                <div class="text-lg font-bold text-center mb-6  text-whiteColor"><br> Top Up!</div>
                <div class="">
                ...
                <button
                    class="inline-block w-full btn btn-success  transition duration-200   ease"
                    id="pay-button">
                    Bayar Sekarang
                </button>
            </div>
        ...
        payButton.addEventListener('click', function(e) {
            ...
            snap.pay('{{ $snapToken }}', {
                ...
                onSuccess: function(result) {
                    ...
                    let url = "{{ url('/buyer/topup/notify-ajax/' . $kodeTopUp . '/success') }}";
                    fetch(url)
                        .then(
                            function() {
                                window.location.href = "{{ url('/buyer/topup/success') }}";
                            }
                        );
                },
                // Optional
                ...
                onError: function(result) {
                    ...
                    const kodetopup = document.getElementById("kode_topup").value;
                    let url = "{{ url('/buyer/topup/notify-ajax/' . $kodeTopUp . '/failed') }}";
                    fetch(url)
                        .then(
                            function() {
                                window.location.href = "{{ url('/buyer/topup/failed') }}";
                            }
                        );
                }
            });
