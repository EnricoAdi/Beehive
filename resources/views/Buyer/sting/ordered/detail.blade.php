...
@section('title', 'Sting Detail Farmer Beehive')

@section('content')
    <a href="{{ url('/buyer/sting/ordered') }}" class="btn btn-secondary w-28 ml-10 mt-4">Back</a>
        ...
        <div class="w-2/3  shadow-gray-300 rounded-lg flex flex-col">
            <div class="text-xl font-semibold flex flex-col gap-2 bg-primary p-5 rounded-t-lg">
                <div class="flex items-center justify-between">
                    <div class="text-lg">{{ $order->sting->TITLE_STING }}
                        <br>

                        <div class="pt-4 pb-2">
                            @foreach ($order->sting->sting_category as $c)
                                <span
                                    class="inline-block bg-secondary rounded-md px-3 py-1
                    ...
                    <div class="flex gap-1 items-center">
                        {{ 'Rp' . number_format($order->COMMISION + $order->TAX, 2, ',', '.') }}
                    </div>
            ...
            <div class="p-5 bg-accent shadow-lg w-full rounded-b-lg text-black flex flex-col gap-5">
                ...
                <div>
                    <div class="text-lg font-semibold">Beeworker : {{ $order->sting->author->NAMA }}
                        ({{ $order->sting->author->EMAIL }})
                    </div>
                    <div class="text-lg font-semibold">{{ $isPremium ? 'Premium' : 'Basic' }} Service</div>
                    <div class="text-lg font-normal">Project Requirement from You : </div>
                    <div class="ml-2 mt-3">
                        {{ $order->REQUIREMENT_PROJECT }}
                    </div>
                    <div class="mt-3">
            ...
            <div class="bg-accent h-fit rounded-lg shadow-lg shadow-gray-300 p-5 sticky top-0 text-black mt-3">
                <div class="flex flex-col gap-3 h-1/2">
                    ...
                    @if ($order->FILENAME_FINAL != '')
                        <a href="{{ url("/buyer/sting/ordered/detail/download/$order->FILENAME_FINAL") }}" target="_blank"
                            ...
                            <div class="flex gap-1">
                                ...
                                @if ($order->STATUS != 3)
                                    ...
                                    <button class="btn btn-success w-1/2"
                                        onclick="accept('{{ $order->ID_TRANSACTION }}')">Accept Work</button>
                                @endif
                            </div>
                        @endif
                    @else
                    ...
                    @forelse ($complains as $c)
                        <div class="mt-4">
                            ...
                            @if ($c->FILE_REVISION != '')
                                ...
                                <a href="{{ url("/buyer/sting/ordered/detail/download/$c->FILE_REVISION") }}"
                                    target="_blank" class="btn btn-secondary w-full mt-2">Download Revision</a>
                            @endif
                        </div>
                        @php
        ...
        <div class="relative w-full max-w-md h-full md:h-auto">
            ...
            <div class="relative bg-base-100  rounded-lg shadow-2xl dark:bg-gray-700">
                ...
                <div class="py-6 px-6 lg:px-8 text-black">
                    <form action="{{ url("/buyer/sting/ordered/detail/decline/$id") }}" method="post">
                        @csrf
...
@section('script')
    <script type="text/javascript">
        ...
        function accept(param) {
            let ask = confirm("Apakah anda yakin untuk menerima hasil kerja order sting ini?");
            if (ask) {
                let navigator = "{{ url('/') }}";
                window.location.href = navigator + "/buyer/sting/ordered/accept/" + param;
            }
        }
