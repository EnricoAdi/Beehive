...
@section('title', 'Detail Sting Beehive')
...
@section('content')
    ...
    <div class="flex justify-between">
        <h1 class="text-3xl ml-20 mt-5">Edit Sting</h1>
        <a href={{ url("/admin/master/sting") }} class="btn btn-secondary mr-20 mt-3">Back</a>
    </div>
    <form method="POST" action="{{url("/admin/master/sting/$sting->ID_STING")}}" enctype="multipart/form-data" class="overflow-y-auto">
        @csrf
        <div class="px-20 mt-10  w-full flex justify-evenly">
            <div class="flex-initial w-1/3">

                {{-- // Thumbnail Sting --}}
                @if (env("APP_ENV")=="production")
                    <img src="{{ asset("sting-thumbnails/$sting->NAMA_THUMBNAIL") }}" alt="" class="object-cover w-full mb-4 h-60 sm:h-52 dark:bg-gray-500 shadow-lg rounded-md">
                @else
                    <img src="{{ asset("storage/sting-thumbnails/$sting->NAMA_THUMBNAIL") }}" alt="" class="object-cover w-full mb-4 h-60 sm:h-52 dark:bg-gray-500 shadow-lg rounded-md">
                @endif
               ...
               <span style="color: red;">{{ $errors->first('foto') }}</span> --}}
                ...
                <button type="submit" class="btn btn-success mt-10 btn-lg w-full h-16">Edit Sting</button>
            </div>
                ...
                <div class="card w-96 text-primary-content mt-3 bg-secondary"  >
                    <div class="card-body">
                        ...
                        <textarea name="desk1" class="textarea textarea-primary w-full max-w-xl" placeholder="Deskripsi Basic Plan"
                            value="{{ $sting->DESKRIPSI_BASIC}}">{{ $sting->DESKRIPSI_BASIC}}</textarea> <span style="color: red;" class="errorInput">{{ $errors->first('desk1') }}</span>
                ...
                <div class="card w-96 text-primary-content mt-3 bg-secondary"  >
                    <div class="card-body">
                        ...
                        <input name="revisi2" type="number" placeholder="Revisi Max"
                            class="input input-bordered w-full max-w-xl" value="{{$sting->MAX_REVISION_PREMIUM}}" /> <span
                        ...
                        <textarea name="desk2" class="textarea textarea-primary w-full max-w-xl" placeholder="Deskripsi Basic Plan"
                            value="{{$sting->DESKRIPSI_PREMIUM}}">{{$sting->DESKRIPSI_PREMIUM}}</textarea> <span style="color: red;" class="errorInput">{{ $errors->first('desk2') }}</span>
                    </div>
                </div>
